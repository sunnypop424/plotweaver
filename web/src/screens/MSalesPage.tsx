  ...
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <div className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-hairline bg-white px-5">
        <button onClick={() => showToast("마켓으로")} className="pw-btn-ghost h-[38px] px-3 text-sm">← 마켓</button>
        <button onClick={() => showToast("신고 메뉴 (R1)")} className="rounded px-3 py-2 text-lg text-[#b4b4b4] transition hover:bg-canvas hover:text-ink2">⋯</button>
      </div>
      ...
      <div className="mx-auto px-6 pb-24 pt-7" style={{ maxWidth: 1080 }}>
        <div style={isWide ? { display: "flex", gap: 32, alignItems: "flex-start" } : { display: "block" }}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <button onClick={() => showToast("1~2화 무료 미리보기를 시작합니다")} className="mt-[22px] h-12 w-full rounded border border-brand bg-white text-[15px] font-bold text-brand transition hover:bg-wash">무료 미리보기 1~2화 ▷</button>
            ...
            <div className="mt-4 rounded-xl border border-hairline bg-white p-5">
              {gated ? (
                ...
                <div>
                  ...
                  <button onClick={() => showToast("읽기 뷰어로 이동합니다")} className="mb-2.5 h-[52px] w-full rounded border-none bg-brand text-base font-bold text-white transition hover:bg-brand-hover">이어서 읽기 (5화)</button>
        ...
        <div className="mt-6">
          ...
          <div className="pw-scroll flex gap-4 overflow-x-auto pb-2.5">
            {OTHER.map((w, i) => (
              <WorkCard key={i} {...w} onOpen={() => showToast(`「${w.title}」로 이동`)} className="w-40 flex-shrink-0" />
            ))}
          </div>
        </div>
      </div>
