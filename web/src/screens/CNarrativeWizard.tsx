...
export default function CNarrativeWizard() {
  ...
  return (
    <div className="min-h-screen bg-canvas">
      <WizardChrome current={2} isMobile={isMobile} onBack={() => showToast("①기본설정으로 돌아갑니다")} onSaveDraft={() => showToast("임시저장됨")} />
        ...
        <div className="min-w-0 flex-1">
          ...
          {!isMobile && (
            <div className="mt-1 flex items-center justify-between gap-3.5">
              <button onClick={() => showToast("①기본설정으로 돌아갑니다")} className="h-14 rounded border border-line2 bg-white px-[22px] text-base font-bold text-ink2 transition hover:border-wash-2 hover:bg-wash hover:text-brand">← 이전: 기본설정</button>
              <div className="flex items-center gap-3.5">
                {!valid && <span className="text-[13px] font-bold text-muted">필수 항목(*)을 채우면 다음으로 넘어갈 수 있어요.</span>}
                <button disabled={!valid} onClick={() => showToast("③관계도 단계로 이동합니다")} className={(valid ? "pw-btn-primary" : "pw-btn-disabled") + " h-14 px-7 text-lg"}>다음: 관계도 →</button>
              </div>
            </div>
          )}
        </div>
      ...
      {isMobile && (
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-hairline bg-white px-4 py-3 shadow-[0_-2px_12px_rgba(0,0,0,0.04)]">
          ...
          <div className="flex gap-2.5">
            <button onClick={() => showToast("①기본설정으로 돌아갑니다")} className="h-[54px] flex-shrink-0 rounded border border-line2 bg-white px-[18px] text-[15px] font-bold text-ink2">← 이전</button>
            <button disabled={!valid} onClick={() => showToast("③관계도 단계로 이동합니다")} className={(valid ? "pw-btn-primary" : "pw-btn-disabled") + " h-[54px] flex-1 text-base"}>다음: 관계도 →</button>
          </div>
        </div>
      )}
