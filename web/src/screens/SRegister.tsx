  ...
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <div className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-hairline bg-white px-5">
        <button onClick={() => showToast("이전 화면으로")} className="pw-btn-ghost h-[38px] px-3 text-sm">← 뒤로</button>
                ...
                return (
                  <div key={i} className="flex items-center justify-between gap-3 border-b border-[#f4f4f4] px-4 py-[13px] last:border-b-0">
                    ...
                    <div className="flex flex-shrink-0 items-center gap-2.5">
                      ...
                      {isAi && (
                        <button onClick={() => showToast(`${c.title} 에디터로 이동 (C4)`)} className="whitespace-nowrap rounded bg-wash px-2.5 py-[7px] text-xs font-bold text-brand transition hover:bg-wash-2">편집 →</button>
                      )}
                    </div>
                  </div>
                );
