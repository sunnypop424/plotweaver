...
export default function C3GeneratingLoader() {
  ...
  // 가짜 진행률 (88% 에서 느려지다 멈춤 — 완료되면 100%로 점프)
  useEffect(() => {
    const progT = window.setInterval(() => {
      if (statusRef.current !== "loading") return;
      setProgress((p) => {
        if (p >= 88) return Math.min(99, p + 0.3);  // 88% 이상에서 매우 느리게, 99%에서 멈춤
        return Math.min(88, p + (2 + Math.random() * 4));
      });
    }, 800);
  ...
  // 실제 API 호출
  useEffect(() => {
    if (calledRef.current || !novelId) return;
    calledRef.current = true;

    generateChapter(novelId, 1)
      .then((res) => {
      ...
      .catch((err: unknown) => {
        setErrorMsg(err instanceof Error ? err.message : "생성에 실패했어요");
        setStatus("error");
      });
  }, [novelId]);
      ...
      <div className="w-full max-w-[560px] rounded-xl border border-hairline bg-white px-8 py-9">
        {isLoading && (
          <div className="flex flex-col items-center gap-[18px] text-center">
            ...
            <div className="text-2xl font-bold tracking-[-0.3px] text-ink">
              ...
              {longWait ? "AI가 열심히 집필 중이에요" : "AI가 1회차를 집필 중입니다"}
        ...
        {isDone && (
          <div className="mt-6 flex flex-col items-center gap-3">
            <div className="text-sm font-bold text-[#2f8f5b]">✓ 1화가 저장됐어요</div>
        ...
        {isError && (
          <button onClick={retry} className="pw-btn-slight mt-6 h-14 w-full text-lg">↻ 다시 시도</button>
        )}
      </div>
    </div>
  );
