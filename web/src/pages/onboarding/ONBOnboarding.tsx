      ...
      {open ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-5" style={{ background: "rgba(18,18,18,0.62)", backdropFilter: "blur(3px)", WebkitBackdropFilter: "blur(3px)", animation: "pw-fade .25s ease" }}>
          <div className="relative w-full max-w-[440px] overflow-hidden rounded-[20px] bg-white" style={{ boxShadow: "0 24px 70px rgba(0,0,0,0.4)", animation: "pw-pop .3s ease" }}>
            {/* skip */}
            <button onClick={() => { setOpen(false); navigate("/library"); }} className="absolute right-4 top-4 z-[5] rounded-full border-none bg-white/70 px-3 py-[7px] text-[13px] font-bold text-muted transition hover:bg-wash hover:text-brand">건너뛰기</button>
            ...
            <div className="px-7 pb-6 pt-[26px]">
              ...
              <div className="mt-5 flex items-center gap-2.5">
                ...
                {idx < last ? (
                  ...
                  <button onClick={() => { setOpen(false); navigate("/create/world"); }} className="inline-flex h-[52px] flex-1 items-center justify-center gap-2 rounded border-none bg-brand text-base font-bold text-white shadow-cta transition hover:bg-brand-hover">✦ 바로 만들기</button>
                )}
              </div>
            </div>
          </div>
