          ...
          <div style={{ animation: "pw-fade .25s ease" }}>
            {emptyLib ? (
              <div className="rounded-2xl border border-dashed border-wash-2 bg-white px-6 py-16 text-center">
                ...
                <button onClick={() => navigate("/market")} className="mt-[22px] h-12 rounded border-none bg-brand px-[22px] text-[15px] font-bold text-white transition hover:bg-brand-hover">마켓 둘러보기</button>
              </div>
            ) : (
              <div className="grid gap-[18px]" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(170px,1fr))" }}>
                {LIBRARY.map((w, i) => (
                  <WorkCard key={i} {...w} onOpen={() => navigate("/read/1")} />
                ))}
              </div>
            )}
          </div>
