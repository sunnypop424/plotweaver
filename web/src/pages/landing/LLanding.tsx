...
export default function LLanding() {
  ...
  /** 라벨 → 실제 라우트 매핑 (랜딩의 모든 진입 동선 연결) */
  const NAV_PATHS: Record<string, string> = {
    "가입 화면으로 이동합니다": "/signup",
    "창작 스튜디오로 이동합니다": "/create/world",
    "마켓으로 이동합니다": "/market",
    "로그인 화면으로 이동합니다": "/login",
    "창작으로 이동합니다": "/create/world",
    "마켓으으로 이동합니다": "/market",
  };
  ...
  const nav = (label: string) => () => { setMenuOpen(false); const p = NAV_PATHS[label]; if (p) navigate(p); else showToast(label); };
      ...
      <div className="bg-toast text-white">
        <div className="mx-auto flex flex-wrap items-start justify-between gap-6 px-6 py-11" style={{ maxWidth: 1180 }}>
          ...
          <div className="flex flex-wrap gap-10">
            <FooterCol title="서비스" links={["창작", "마켓"]} onClick={(l) => navigate(l === "창작" ? "/create/world" : "/market")} />
            <FooterCol title="정보" links={["이용약관", "개인정보처리방침", "문의하기"]} onClick={(l) => navigate(l === "이용약관" ? "/terms" : l === "개인정보처리방침" ? "/privacy" : "/contact")} />
          </div>
        </div>
